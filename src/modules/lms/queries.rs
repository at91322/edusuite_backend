// src/modules/lms/queries.rs

use sqlx::Row;
use uuid::Uuid;

use crate::error::AppError;
use super::models::{
    CourseSectionsParams, ListSectionsParams, SectionDetail, SectionSummary,
};

// ── GET /lms/sections (existing, unchanged) ───────────────────────────────────

pub async fn list_sections(
    tx:     &mut sqlx::Transaction<'_, sqlx::Postgres>,
    params: &ListSectionsParams,
) -> Result<(Vec<SectionSummary>, i64), AppError> {

    let total: i64 = sqlx::query_scalar!(
        r#"
        SELECT COUNT(*)::bigint AS "count!"
        FROM sis.course_sections cs
        WHERE cs.tenant_id  = current_setting('app.current_tenant_id', true)::uuid
          AND cs.is_current = true
          AND ($1::uuid IS NULL OR cs.term_id       = $1)
          AND ($2::uuid IS NULL OR cs.course_id     = $2)
          AND ($3::uuid IS NULL OR cs.instructor_id = $3)
          AND ($4::bool IS NULL OR cs.is_published  = $4)
        "#,
        params.term_id       as Option<Uuid>,
        params.course_id     as Option<Uuid>,
        params.instructor_id as Option<Uuid>,
        params.is_published  as Option<bool>,
    )
    .fetch_one(&mut **tx)
    .await
    .map_err(AppError::from)?;

    let rows = sqlx::query(
        r#"
        SELECT
            cs.id                               AS id,
            cs.section_number                   AS section_number,
            cs.course_id                        AS course_id,
            c.subject                           AS course_subject,
            c.course                            AS course_number,
            c.title                             AS course_title,
            cs.term_id                          AS term_id,
            t.name                              AS term_name,
            t.status::text                      AS term_status,
            cs.instructor_id                    AS instructor_id,
            u.first_name || ' ' || u.last_name  AS instructor_name,
            cs.instructional_format::text       AS instructional_format,
            cs.max_enrollment                   AS max_enrollment,
            cs.is_published                     AS is_published,
            COUNT(e.id)                         AS enrolled_count
        FROM sis.course_sections cs
        JOIN sis.courses c        ON c.id  = cs.course_id
        JOIN sis.academic_terms t ON t.id  = cs.term_id
        JOIN core.users u         ON u.id  = cs.instructor_id
        LEFT JOIN sis.enrollments e
               ON e.section_id = cs.id
              AND e.status = 'enrolled'
        WHERE cs.tenant_id  = current_setting('app.current_tenant_id', true)::uuid
          AND cs.is_current = true
          AND ($1::uuid IS NULL OR cs.term_id       = $1)
          AND ($2::uuid IS NULL OR cs.course_id     = $2)
          AND ($3::uuid IS NULL OR cs.instructor_id = $3)
          AND ($4::bool IS NULL OR cs.is_published  = $4)
        GROUP BY
            cs.id, cs.section_number, cs.course_id,
            c.subject, c.course, c.title,
            cs.term_id, t.name, t.status, t.start_date,
            cs.instructor_id, u.first_name, u.last_name,
            cs.instructional_format, cs.max_enrollment, cs.is_published
        ORDER BY t.start_date DESC, c.subject ASC, c.course ASC, cs.section_number ASC
        LIMIT  $5
        OFFSET $6
        "#,
    )
    .bind(params.term_id)
    .bind(params.course_id)
    .bind(params.instructor_id)
    .bind(params.is_published)
    .bind(params.per_page())
    .bind(params.offset())
    .fetch_all(&mut **tx)
    .await
    .map_err(AppError::from)?;

    let sections = rows.iter().map(|r| -> Result<SectionSummary, AppError> {
        Ok(SectionSummary {
            id:                   r.try_get("id").map_err(AppError::from)?,
            section_number:       r.try_get("section_number").map_err(AppError::from)?,
            course_id:            r.try_get("course_id").map_err(AppError::from)?,
            course_subject:       r.try_get("course_subject").map_err(AppError::from)?,
            course_number:        r.try_get("course_number").map_err(AppError::from)?,
            course_title:         r.try_get("course_title").map_err(AppError::from)?,
            term_id:              r.try_get("term_id").map_err(AppError::from)?,
            term_name:            r.try_get("term_name").map_err(AppError::from)?,
            term_status:          r.try_get("term_status").map_err(AppError::from)?,
            instructor_id:        r.try_get("instructor_id").map_err(AppError::from)?,
            instructor_name:      r.try_get("instructor_name").map_err(AppError::from)?,
            instructional_format: r.try_get("instructional_format").map_err(AppError::from)?,
            max_enrollment:       r.try_get("max_enrollment").map_err(AppError::from)?,
            is_published:         r.try_get("is_published").map_err(AppError::from)?,
            enrolled_count:       r.try_get("enrolled_count").map_err(AppError::from)?,
        })
    }).collect::<Result<Vec<_>, _>>()?;

    Ok((sections, total))
}

// GET /lms/sections/:id ────────────────────────────────────────────────────────

/// Fetch full section detail by section UUID.
///
/// Returns None if the section does not exist in this tenant (RLS hides it or
/// it is genuinely absent) — the handler maps this to 404.
///
/// A single query collects all fields using scalar sub-selects for the counts
/// rather than GROUP BY, making the result deterministic and avoiding the
/// explosion in rows that JOINs on one-to-many relationships would produce.
pub async fn get_section(
    tx:         &mut sqlx::Transaction<'_, sqlx::Postgres>,
    section_id: Uuid,
) -> Result<Option<SectionDetail>, AppError> {

    let row = sqlx::query(
        r#"
        SELECT
            cs.id                                                     AS id,
            cs.section_number                                         AS section_number,

            -- Course
            cs.course_id                                              AS course_id,
            c.subject                                                 AS course_subject,
            c.course                                                  AS course_number,
            c.title                                                   AS course_title,
            COALESCE(c.credits, 0.0)                                  AS course_credits,

            -- Term
            cs.term_id                                                AS term_id,
            t.name                                                    AS term_name,
            t.status::text                                            AS term_status,
            t.start_date                                              AS term_start_date,
            t.end_date                                                AS term_end_date,

            -- Instructor
            cs.instructor_id                                          AS instructor_id,
            u.first_name || ' ' || u.last_name                       AS instructor_name,

            -- Section metadata
            cs.instructional_format::text                             AS instructional_format,
            cs.instruction_language                                   AS instruction_language,
            cs.max_enrollment                                         AS max_enrollment,
            cs.waitlist_capacity                                      AS waitlist_capacity,
            cs.is_published                                           AS is_published,
            cs.effective_start_date                                   AS effective_start_date,

            -- Live counts (scalar sub-selects avoid GROUP BY complexity)
            (SELECT COUNT(*) FROM sis.enrollments e
              WHERE e.section_id = cs.id
                AND e.tenant_id  = cs.tenant_id
                AND e.status     = 'enrolled')::bigint                AS enrolled_count,

            (SELECT COUNT(*) FROM sis.enrollments e
              WHERE e.section_id = cs.id
                AND e.tenant_id  = cs.tenant_id
                AND e.status     = 'waitlisted')::bigint              AS waitlisted_count,

            (SELECT COUNT(*) FROM lms.course_modules cm
              WHERE cm.section_id = cs.id
                AND cm.tenant_id  = cs.tenant_id)::bigint             AS module_count,

            (SELECT COUNT(*) FROM lms.assignments a
              WHERE a.section_id = cs.id
                AND a.tenant_id  = cs.tenant_id)::bigint              AS assignment_count

        FROM sis.course_sections cs
        JOIN sis.courses c        ON c.id = cs.course_id
        JOIN sis.academic_terms t ON t.id = cs.term_id
        JOIN core.users u         ON u.id = cs.instructor_id
        WHERE cs.id        = $1
          AND cs.tenant_id = current_setting('app.current_tenant_id', true)::uuid
          AND cs.is_current = true
        "#,
    )
    .bind(section_id)
    .fetch_optional(&mut **tx)
    .await
    .map_err(AppError::from)?;

    match row {
        None => Ok(None),
        Some(r) => {
            let credits_raw: f64 = r.try_get::<f64, _>("course_credits")
                .or_else(|_| r.try_get::<i32, _>("course_credits").map(|v| v as f64))
                .unwrap_or(0.0);

            Ok(Some(SectionDetail {
                id:                   r.try_get("id").map_err(AppError::from)?,
                section_number:       r.try_get("section_number").map_err(AppError::from)?,
                course_id:            r.try_get("course_id").map_err(AppError::from)?,
                course_subject:       r.try_get("course_subject").map_err(AppError::from)?,
                course_number:        r.try_get("course_number").map_err(AppError::from)?,
                course_title:         r.try_get("course_title").map_err(AppError::from)?,
                course_credits:       credits_raw,
                term_id:              r.try_get("term_id").map_err(AppError::from)?,
                term_name:            r.try_get("term_name").map_err(AppError::from)?,
                term_status:          r.try_get("term_status").map_err(AppError::from)?,
                term_start_date:      r.try_get("term_start_date").map_err(AppError::from)?,
                term_end_date:        r.try_get("term_end_date").map_err(AppError::from)?,
                instructor_id:        r.try_get("instructor_id").map_err(AppError::from)?,
                instructor_name:      r.try_get("instructor_name").map_err(AppError::from)?,
                instructional_format: r.try_get("instructional_format").map_err(AppError::from)?,
                instruction_language: r.try_get("instruction_language").map_err(AppError::from)?,
                max_enrollment:       r.try_get("max_enrollment").map_err(AppError::from)?,
                waitlist_capacity:    r.try_get("waitlist_capacity").map_err(AppError::from)?,
                is_published:         r.try_get("is_published").map_err(AppError::from)?,
                effective_start_date: r.try_get("effective_start_date").map_err(AppError::from)?,
                enrolled_count:       r.try_get("enrolled_count").map_err(AppError::from)?,
                waitlisted_count:     r.try_get("waitlisted_count").map_err(AppError::from)?,
                module_count:         r.try_get("module_count").map_err(AppError::from)?,
                assignment_count:     r.try_get("assignment_count").map_err(AppError::from)?,
            }))
        }
    }
}

// GET /sis/courses/:id/sections ────────────────────────────────────────────────

/// List all sections for a specific course. Mounts under /sis/courses/:id/sections
/// but lives here because the data shape (LMS-enriched SectionSummary) is
/// identical to list_sections — we reuse the same response type.
///
/// Returns (sections, total). Returns a 404 via the handler if the course_id
/// does not exist in this tenant before running the main query.
pub async fn list_sections_for_course(
    tx:        &mut sqlx::Transaction<'_, sqlx::Postgres>,
    course_id: Uuid,
    params:    &CourseSectionsParams,
) -> Result<(Vec<SectionSummary>, i64), AppError> {

    // Guard: confirm course exists in this tenant (RLS-scoped).
    let course_exists: bool = sqlx::query_scalar!(
        r#"
        SELECT EXISTS(
            SELECT 1 FROM sis.courses
            WHERE id        = $1
              AND tenant_id = current_setting('app.current_tenant_id', true)::uuid
        ) AS "exists!"
        "#,
        course_id,
    )
    .fetch_one(&mut **tx)
    .await
    .map_err(AppError::from)?;

    if !course_exists {
        return Err(AppError::NotFound(format!("Course {} not found", course_id)));
    }

    let total: i64 = sqlx::query_scalar!(
        r#"
        SELECT COUNT(*)::bigint AS "count!"
        FROM sis.course_sections cs
        WHERE cs.course_id  = $1
          AND cs.tenant_id  = current_setting('app.current_tenant_id', true)::uuid
          AND cs.is_current = true
          AND ($2::uuid IS NULL OR cs.term_id       = $2)
          AND ($3::uuid IS NULL OR cs.instructor_id = $3)
          AND ($4::bool IS NULL OR cs.is_published  = $4)
        "#,
        course_id,
        params.term_id       as Option<Uuid>,
        params.instructor_id as Option<Uuid>,
        params.is_published  as Option<bool>,
    )
    .fetch_one(&mut **tx)
    .await
    .map_err(AppError::from)?;

    let rows = sqlx::query(
        r#"
        SELECT
            cs.id                               AS id,
            cs.section_number                   AS section_number,
            cs.course_id                        AS course_id,
            c.subject                           AS course_subject,
            c.course                            AS course_number,
            c.title                             AS course_title,
            cs.term_id                          AS term_id,
            t.name                              AS term_name,
            t.status::text                      AS term_status,
            cs.instructor_id                    AS instructor_id,
            u.first_name || ' ' || u.last_name  AS instructor_name,
            cs.instructional_format::text       AS instructional_format,
            cs.max_enrollment                   AS max_enrollment,
            cs.is_published                     AS is_published,
            COUNT(e.id)                         AS enrolled_count
        FROM sis.course_sections cs
        JOIN sis.courses c        ON c.id = cs.course_id
        JOIN sis.academic_terms t ON t.id = cs.term_id
        JOIN core.users u         ON u.id = cs.instructor_id
        LEFT JOIN sis.enrollments e
               ON e.section_id = cs.id
              AND e.status = 'enrolled'
        WHERE cs.course_id  = $1
          AND cs.tenant_id  = current_setting('app.current_tenant_id', true)::uuid
          AND cs.is_current = true
          AND ($2::uuid IS NULL OR cs.term_id       = $2)
          AND ($3::uuid IS NULL OR cs.instructor_id = $3)
          AND ($4::bool IS NULL OR cs.is_published  = $4)
        GROUP BY
            cs.id, cs.section_number, cs.course_id,
            c.subject, c.course, c.title,
            cs.term_id, t.name, t.status, t.start_date,
            cs.instructor_id, u.first_name, u.last_name,
            cs.instructional_format, cs.max_enrollment, cs.is_published
        ORDER BY t.start_date DESC, cs.section_number ASC
        LIMIT  $5
        OFFSET $6
        "#,
    )
    .bind(course_id)
    .bind(params.term_id)
    .bind(params.instructor_id)
    .bind(params.is_published)
    .bind(params.per_page())
    .bind(params.offset())
    .fetch_all(&mut **tx)
    .await
    .map_err(AppError::from)?;

    let sections = rows.iter().map(|r| -> Result<SectionSummary, AppError> {
        Ok(SectionSummary {
            id:                   r.try_get("id").map_err(AppError::from)?,
            section_number:       r.try_get("section_number").map_err(AppError::from)?,
            course_id:            r.try_get("course_id").map_err(AppError::from)?,
            course_subject:       r.try_get("course_subject").map_err(AppError::from)?,
            course_number:        r.try_get("course_number").map_err(AppError::from)?,
            course_title:         r.try_get("course_title").map_err(AppError::from)?,
            term_id:              r.try_get("term_id").map_err(AppError::from)?,
            term_name:            r.try_get("term_name").map_err(AppError::from)?,
            term_status:          r.try_get("term_status").map_err(AppError::from)?,
            instructor_id:        r.try_get("instructor_id").map_err(AppError::from)?,
            instructor_name:      r.try_get("instructor_name").map_err(AppError::from)?,
            instructional_format: r.try_get("instructional_format").map_err(AppError::from)?,
            max_enrollment:       r.try_get("max_enrollment").map_err(AppError::from)?,
            is_published:         r.try_get("is_published").map_err(AppError::from)?,
            enrolled_count:       r.try_get("enrolled_count").map_err(AppError::from)?,
        })
    }).collect::<Result<Vec<_>, _>>()?;

    Ok((sections, total))
}